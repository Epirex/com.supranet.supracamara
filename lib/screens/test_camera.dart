...
class CameraExampleHome extends StatefulWidget {
  /// Default Constructor
  const CameraExampleHome({Key? key, required this.data}) : super(key: key);

  final Map<String, dynamic> data;

  @override
  State<CameraExampleHome> createState() {
    return _CameraExampleHomeState(data: data);
  }
}
...
class _CameraExampleHomeState extends State<CameraExampleHome>
  ...
  _CameraExampleHomeState({Key? key, required this.data});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: AppColors.black,
        centerTitle: true,
        title: Text('${_printDuration(Duration(seconds: _start))} ',//'Camera example',
  ...
  Widget _cameraPreviewWidget() {
    ...
    if (cameraController == null || !cameraController.value.isInitialized) {
      return const Text(
        'Seleccioná una cámara',//'Tap a camera',
        style: TextStyle(
          color: Colors.white,
          fontSize: 24.0,
  ...
  Future _thumbnailWidget() async {
    final appStorage = await getApplicationDocumentsDirectory();
    final file = File('${appStorage.path}/${videoFile?.name}');
    final raf = await file.open(mode: FileMode.write);
    final list = await videoFile?.readAsBytes();
    await raf.writeFrom(list?.toList() ?? []);
    await raf.close();
    if (!mounted) return;
    _timer.cancel();
    ...
    try {
      if (controller != null) {
        ...
        if (cameraController.value.isCaptureOrientationLocked) {
          ...
          Get.find<SelectItemController>().isHorizontal.value = cameraController.value.isCaptureOrientationLocked;
        } else {
...
class CameraApp extends StatefulWidget {
  /// Default Constructor
  CameraApp({Key? key, required this.data}) : super(key: key);
